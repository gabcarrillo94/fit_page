...
<div class="form-style-5">
			...
			<div style="display:inline-block;width: 100%;margin-bottom:4%">
                <div style="padding-left: 5%;">
                    <label for="rdo-1" class="btn-radio">
						...
						id="rdo-1" name="metric_system" value="IS"
						@if($data->metric_system == 'IS') checked @endif>
                    ...
                    <label for="rdo-2" class="btn-radio">
						...
						@if($data->metric_system == 'ES') checked @endif>
                            <svg width="20px" height="20px" viewBox="0 0 20 20">
                                <circle cx="10" cy="10" r="9"></circle>
                                <path d="M10,7 C8.34314575,7 7,8.34314575 7,10 C7,11.6568542 8.34314575,13 10,13 C11.6568542,13 13,11.6568542 13,10 C13,8.34314575 11.6568542,7 10,7 Z" class="inner"></path>
                                <path d="M10,1 L10,1 L10,1 C14.9705627,1 19,5.02943725 19,10 L19,10 L19,10 C19,14.9705627 14.9705627,19 10,19 L10,19 L10,19 C5.02943725,19 1,14.9705627 1,10 L1,10 L1,10 C1,5.02943725 5.02943725,1 10,1 L10,1 Z" class="outer"></path>
		...
		<fieldset>
            <legend><span class="number">2</span> Parrillo Caliper Method (mm)</legend>
			
			{{ Form::open(['route' => 'data.calculatorpcm']) }}
			{{ csrf_field() }}
			<input id="pcm_metric_system" type="hidden" name="metric_system"
				value="{{ $data->metric_system or old('metric_system') }}">
		...
		<fieldset>
			...
			{{ csrf_field() }}
			<input id="jp7_metric_system" type="hidden" name="metric_system" 
				value="{{ $data->metric_system or old('metric_system') }}">
		...
		<fieldset>
            <legend>
				<span class="number">4</span> Navy Tape Measure Method
					<span id="unity_label">
					@if(empty($data->metric_system) || $data->metric_system == 'IS')
						(Cm)
					@elseif($data->metric_system == 'ES')
						(In)
					@endif
					</span>
			</legend>
			...
			{{ csrf_field() }}
			<input id="ntm_metric_system" type="hidden" name="metric_system" 
				value="{{ $data->metric_system or old('metric_system') }}">
			
			<label for="height_integer" id="height_integer_label">
				Estatura @if(empty($data->metric_system) || $data->metric_system == 'IS') (Mt):
						 @elseif($data->metric_system == 'ES') (Ft):
						 @endif
			</label>
			<input type="number" class="form-control"
			...
			<label for="height_decimal" id="height_decimal_label">
				Estatura @if(empty($data->metric_system) || $data->metric_system == 'IS') (Cm):
						 @elseif($data->metric_system == 'ES') (In):
						 @endif
			</label>
			<input type="number" class="form-control"
				name="height_decimal" value="{{ $data->height_decimal or old('height_decimal') }}"
				min="0" id="height_decimal" placeholder="00">
